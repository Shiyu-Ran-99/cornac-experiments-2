...
class GiniCoeff(DiversityMetric):
    ...
    def __init__(self, item_genre, k=-1):
        ...
        **kwargs: For compatibility
        ...
        for x in truncated_pd_rank:
            if x in self.item_genre.keys():
                recommend_items_genre.append(self.item_genre[x].tolist())
        proportion = []
...
def relevance(rel, rating_threshold=1):
    """Compute the probability of items being liked by a heuristic mapping using rating values.
    ...
    # return [(2**max(0, i - rating_threshold) -1)/g_max for i in rel] #paper formula
    # actual modification in experiment
    return [(2**max(0, i - rating_threshold))/g_max for i in rel]


def discount(x, disc_type="exponential", base=0.9):
    """Compute the probability of recommended items been discovered in a list.
...
class EILD(DiversityMetric):
    ...
    def __init__(self, item_feature,  k=-1, disc_type="exponential", base=0.9):
        ...
        if isinstance(disc_type, str) and disc_type.lower() in ["exponential", "logarithmic",
            ...
            # by default using exponential discount.
    ...
    def compute(self, pd_rank, gt_ratings, rating_threshold=1.0, **kwargs):
        ...
        **kwargs: For compatibility
        ...
        pd_vec0 = [self.item_feature[i].tolist() if i in self.item_feature.keys() else [
        ] for i in truncated_pd_rank]
        pd_vec = [x for x in pd_vec0 if x]
        # for i in range (len(truncated_pd_rank)):
        try:
            for i in range(len(pd_vec)):
                ieild = 0
                inorm = 0
                # for j in range (len(truncated_pd_rank)):
...
class ILD(DiversityMetric):
    ...
    def __init__(self, item_feature, k=-1, distance_type="cosine"):
    ...
    def compute(self, pd_rank, **kwargs):
        ...
        **kwargs: For compatibility
        ...
        # error when pd_vec contains [] cannot proceed.  add the following line to remove [].
        pd_vec = [x for x in pd_vec0 if x]
        try:
...
class Binomial(DiversityMetric):
    ...
    def __init__(self, item_genre, k=-1, alpha=0.9):
    ...
    def globalFeatureProbs(self, user_history):
        ...
        # if the count doesn't have an item, need special handling.
    ...
    def userFeatureProbs(self, globalProb, user_history):
        ...
        """
        # globalProb = globalFeatureProbs(user_history,item_genre)
        userProbs = []
        if (self.alpha == 0):
            userProbs = globalProb
            return userProbs
        elif self.alpha > 0 and self.alpha <= 1:
            all_genre = [self.item_genre[x].tolist() if x in self.item_genre else [
        ...
        **kwargs: For compatibility
        ...
        try:
            ...
            NonRedundacy = self.binomialNonRedundancy(
                truncated_pd_rank.tolist(), userProb)
...
def compute_distr_category(arr, discount=False):
    ...
    for indx, item in enumerate(arr):
        ...
        distr[item] = story_freq + 1 * 1 / rank / \
            sum_one_over_ranks if discount else story_freq + 1 * 1 / n
        count += 1
    return distr
...
def compute_distr_majority_minority(arr,  discount=False):
    ...
    Parameters
    ----------
        arr: numpy array of list [number_of_minority_mentions, number_of_majority_mentions].
        discount: boolean. false if no discount, true if discounted
    Returns
    -------
        Dictionary. distribution of pool or recommendation. 
    ...
    #   for i in value.keys():
    #     all_entities += [i] if i not in all_entities else [] ##without duplicates
    for idx, value in arr.items():
        rank = count + 1
    ...
    def compute(self, pd_rank, pool, **kwargs):
        ...
        **kwargs: For compatibility
        ...
        try:
            ...
            # pool always don't consider rank.
...
class Calibration(DiversityMetric):
    ...
    def __init__(self, item_feature, data_type, k=-1, divergence_type="KL", discount=False, n_bins=5):
        ...
        # self.item_feature: category or complexity.
        if isinstance(item_feature, dict):
            self.item_feature = item_feature
        else:
            raise ValueError(
        ...
        if self.data_type == "category":
            try:
                ...
                cal = compute_divergence(
                    freq_history, freq_rec, divergence_type=self.divergence_type)
                return cal
            except ValueError:
                return None
...
class Representation(DiversityMetric):
    ...
    def compute(self, pd_rank, pool, **kwargs):
        ...
        **kwargs: For compatibility
        ...
        # if len(recommendation_entities)>0 and len(pool_entities)>0 :
        try:
        ...
        # else:
            ...
            # raise ValueError(
            #     "Representation computation requires at least one recommendation item has entities information."
            # )
            # print("cannot get value for representation")
            return None
...
class AlternativeVoices(DiversityMetric):
    ...
    def __init__(self, item_minor_major, data_type='mainstream',  k=-1, divergence_type="KL", discount=False):
        ...
        # DiversityMetric.__init__(self, name="AlternativeVoices@{}".format(k), k=k)
    ...
    # print(pool_entities)
        # if len(pool_minor_major)>0 and len(recommendation_minor_major)>0 :
        try:
            ...
            divergence = np.nan
            if recommendation_distr[0] != 0 and recommendation_distr[1] != 0:
                divergence = compute_divergence(
                    recommendation_distr, pool_distr,  divergence_type=self.divergence_type)
            return divergence
        except ValueError:
            # raise ValueError(
            #     "Alternative Voices computation requires at least one recommendation item has majority and minority score ."
            # )
            return None
