...
class TestExperiment(unittest.TestCase):
    ...
    def test_with_paper_data(self):
        ...
        bm = BaseMethod.from_splits(
            train_data=train_data, test_data=test_data, val_data=None, fmt='UIR',
            rating_threshold=0, exclude_unknowns=False, seed=None, verbose=True)
        # Experiment(
        #     eval_method=bm,
        #     models=[ENMF(num_epochs=5)],
        #     metrics=[Recall(50), Recall(100), Recall(
        #         200), NDCG(50), NDCG(100), NDCG(200)],
        #     verbose=True,
        # ).run()
    ...
    def test_mind(self):
        ...
        # print(entities)
        # entities1 = mind.load_entities(fpath="./tests/entities_example.csv")
        # print(entities1)
    ...
    def test_with_diversity(self):
        ...
        #     models= [UserKNN(k=3, similarity="pearson", name="UserKNN-Pearson")],
        #     metrics=[ndcg, gini,eild,ild,bino],
        #     verbose=True,
        # ).run()
        Experiment(eval_method=ratio_split,
                   models=[UserKNN(k=3, similarity="pearson",
                                   name="UserKNN"), PMF(1, 0)],
                   metrics=[gini, eild, ild, bino, RMSE(), Recall()],
                   verbose=True,
                   ).run()
        # Radio metrics
        # Experiment(eval_method=ratio_split,
        #            models=[UserKNN(k=3, similarity="pearson",
        #                            name="UserKNN-Pearson")],
        #            metrics=[act, cal, frag, repre, alt],
        #            verbose=True,
        #            ).run()
